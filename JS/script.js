    ...
    function updateVariantPrice() {
      ...
      product.variants.forEach((variantGroup, groupIndex) => {
        ...
        if (select && select.value) {
          ...
          if (variantGroup.priceAdd !== undefined && select.value === variantGroup.options.find(opt => opt === 'Ya' || opt === 'Yes')) { // Assume "Ya" triggers add
    ...
    function confirmAddWithVariants() {
      ...
      // Add to cart
      const existingItem = cart.find(item => item.id === product.id && JSON.stringify(item.selectedVariants || {}) === JSON.stringify(selectedVariants));
    ...
    function addToCart(productId) {
      const product = products.find(p => p.id === productId);
      const existingItem = cart.find(item => item.id === productId && !item.selectedVariants);
        ...
        return `
          <div class="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
            ...
            <div class="flex items-center space-x-3">
              <button onclick="updateQuantity(${item.id}, -1, ${JSON.stringify(item.selectedVariants || {})})" class="w-8 h-8 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center">-</button>
              <span class="font-semibold">${item.quantity}</span>
              <button onclick="updateQuantity(${item.id}, 1, ${JSON.stringify(item.selectedVariants || {})})" class="w-8 h-8 rounded-full bg-gray-200 hover:bg-gray-300 flex items-center justify-center">+</button>
              <button onclick="removeFromCart(${item.id}, ${JSON.stringify(item.selectedVariants || {})})" class="ml-4 text-red-500 hover:text-red-700">
    ...
    function updateQuantity(productId, change, selectedVariants) {
      const itemKey = JSON.stringify(selectedVariants || {});
      const item = cart.find(i => i.id === productId && JSON.stringify(i.selectedVariants || {}) === itemKey);
      if (item) {
    ...
    function removeFromCart(productId, selectedVariants) {
      const itemKey = JSON.stringify(selectedVariants || {});
      cart = cart.filter(item => !(item.id === productId && JSON.stringify(item.selectedVariants || {}) === itemKey));
      updateCartCount();
      renderCart();
    ...
    document.getElementById('checkout-btn').addEventListener('click', () => {
      ...
      cart.forEach(item => {
        ...
        itemLine += `  ${item.quantity} x Rp ${displayPrice.toLocaleString('id-ID')} = Rp ${itemTotal.toLocaleString('id-ID')}%0A`;
        message += itemLine;
      });
